 ...
 * 
 * Given a file path for basketball statistics, the program will sort them in ascending order according
 * to their player ID, and will write those results to a binary file. There are no user inputs for this program
 * and the path is given as the fist command line argument.
 */
...
public class Proj1A {
	...
	/*
	 * gets the filename of the specified file path
	 * 
	 * paramters - a string that represents the path to the file to be read
	 * returns - a string of the new filename (no path or extension)
	 */
	...
	/*
	 * reads in the data from the given file and parses it. splits the data into categories, creates record
	 * objects and sorts by the player id
	 * 
	 * paramaters - a string representing the file path
	 * returns - an array of all the record objects created in sorted order
	 */
	...
	/*
	 * writes all of the data to the binary file with padded spaces
	 * 
	 * paramaters-a string representing the file name to write to as the binary file and an arry of record 
    ...
    private static void WriteToBinaryFile(String file_name, Record[] lines_list)
    {
		...
		for (int i = 0; i < lines_list.length; i++) {
			Record record = lines_list[i];
			if (record != null) {
				record.DumpObject(bin_file);
			}
		}	
